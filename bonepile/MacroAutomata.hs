...
type MacroTrans f q g = forall a i . q a i -> f (q (Context g a) :->: a) i -> Context g a i
...
runMacroTrans :: (HFunctor g, HFunctor f, HFunctor q) =>
                 MacroTrans f q g -> q (Cxt h g a) i -> Cxt h f a i -> Cxt h g a i
...
runMacroTrans' :: forall g f q h a i .
                  (HFunctor g, HFunctor f, HFunctor q) => MacroTrans f q g -> q (Cxt h g a) i
               -> Cxt h f (q (Cxt h g a) :->: a) i -> Cxt h g a i
runMacroTrans' tr q t = run t q where
...
compDownMacro :: forall f g h q p . (HFunctor f, HFunctor g, HFunctor h, Functor q)
...
type MacroTransId  f g = forall a i . a i -> f (Context g a :->: a) i -> Context g a i
...
type MacroTransId' f g = forall a i . Context g a i -> f (Context g a :->: Context g a) i -> Context g a i
...
type MacroTransLA  f q p g = forall a i . q a i -> p -> f ((q (Context g a) :->: a) :*: K p) i -> Context g a i

-- | This type is a more convenient variant of 'MacroTransLA' with
-- which one can avoid using 'Hole' explicitly when injecting
-- placeholders in the result.
type MacroTransLA' f q p g = forall a i . q (Context g a) i -> p ->
                             f ((q (Context g a) :->: Context g a) :*: K p) i -> Context g a i
